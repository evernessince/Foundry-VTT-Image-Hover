...
export class Settings {
  static createSettings() {
    ...
    game.settings.register("image-hover", "excludeTags", {
      name: "Exclude tiles / tokens with these tags",
      hint: "Comma-separated Tagger tags. Tiles and tokens carrying any of these tags will be skipped by the image hover popup. Requires the Tagger module (https://github.com/fantasycalendar/FoundryVTT-Tagger) — without it this setting has no effect.",
      scope: "world",
      config: true,
      restricted: true,
      type: String,
      ...
      onChange: () => {
        if (!game.modules.get("tagger")?.active) {
          ui.notifications?.warn(
            "Image Hover: 'Exclude tiles / tokens with these tags' requires the Tagger module. The value has been saved but will have no effect until Tagger is installed and enabled."
          );
        }
      },
    });
    ...
    // client setting — optional decorative graphic layered on the popup
    ...
    // client setting — graphic size relative to the popup
    ...
    // client setting — graphic horizontal offset
    ...
    // client setting — graphic vertical offset
    ...
    // client setting — graphic stacking order
    ...
    // client setting — scale of the token art inside the popup
